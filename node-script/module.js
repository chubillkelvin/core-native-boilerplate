...
const available1stLevelModuleNames = fs.readdirSync(path.resolve(__dirname, `../${srcFolderName}/module`), {withFileTypes: true}).filter(_ => _.isDirectory() && _.name !== "main").map(_ => _.name);
...
function createModuleFolder(moduleNameInArg) {
    ...
    const moduleName = moduleNameInArg.split("/")[0];
    const subModuleName = moduleNameInArg.split("/")[1];
    const sourcePath = path.resolve(__dirname, "./module-template");
    ...
    executeReplace("/hooks.ts", [`${modulePascalName}State`, moduleFullName]);
...
function updateRootState(moduleNameInArg) {
    ...
    const replacedRootStateFileContent =
        ...
        rootStateFileContent.substr(0, lastStateDeclarationIndex) +
...
function generate() {
    ...
    try {
        ...
        if (available1stLevelModuleNames.indexOf(splitModuleNames[0]) === -1) throw new Error(`Module [${moduleNameInArg.split("/")[0]}] must be one of: [${available1stLevelModuleNames.join("/")}]`);
