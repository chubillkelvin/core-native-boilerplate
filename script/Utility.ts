...
export class Utility {
    ...
    static async prepareFolder(folderPath: string) {
        Utility.print("Preparing folder: " + folderPath);

        const folderExist = await fs.pathExists(folderPath);
        if (!folderExist) await fs.mkdir(folderPath);
     ...
     */
    static replaceTemplate(filePath: string, replacedContents: string[]) {
        try {
            let fileContent = fs.readFileSync(filePath).toString();
            replacedContents.forEach((content, i) => (fileContent = fileContent.replace(new RegExp("\\{" + (i + 1) + "\\}", "g"), content)));
            fs.writeFileSync(filePath, fileContent);
        } catch (error) {
            Utility.printErrorThenExit(error);
        }
    }

    static debug() {
        Utility.runProcess("open", ["rndebugger://set-debugger-loc?host=localhost&port=8081"]);
    }
}
